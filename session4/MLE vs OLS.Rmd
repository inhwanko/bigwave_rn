...
$$
\mu_i = \mathbf{x}_i\beta, \ \ \sigma^2_i = \mathrm{exp}(\mathbf{x_i}\gamma)
$$
...
\sigma^2_i & = \mathrm{exp}(1+0x_{1i}+3x_{2i}) 
\end{aligned}
$$
...
$$
$$
\pi_i = \mathrm{logit}^{-1}(X_i\beta) = \frac{e^{X_i\beta}}{1+e^{X_i\beta}} = \frac{1}{1+e^{-X_i\beta}}
$$
...
$$
\begin{aligned}
\mathcal{L}(\pi|\mathrm{y}) & \propto \prod^n_{i=1}\pi_i^{y_i}(1-\pi_i)^{1-y_i} \\
\mathcal{L}(\beta|\mathrm{y}) & \propto \prod^n_{i=1}(\frac{1}{1+\mathrm{exp}(-\mathrm{x}_i\beta)})^{y_i}(1-\frac{1}{1+\mathrm{exp}(-\mathrm{x}_i\beta)})^{1-y_i} \\
\mathcal{L}(\beta|\mathrm{y}) & \propto \prod^n_{i=1}(1+\mathrm{exp}(-\mathrm{x}_i\beta))^{-y_i}(1+\mathrm{exp}(\mathrm{x}_i\beta))^{-(1-y_i)} \\
\mathrm{log}\mathcal{L}(\beta|\mathrm{y}) & \propto \sum^n_{i=1}y_i\mathrm{log}(1+\mathrm{exp}(-\mathrm{x}_i\beta))-(1-y_i)\mathrm{log}(1+\mathrm{exp{(\mathrm{x}_i\beta)}})
\end{aligned}
$$
...
$$
\pi_i = \mathrm{logit}^{-1}(0.5 + x_{1i} +1.5x_{2_i}) = \frac{e^{X_i\beta}}{1+e^{X_i\beta}} = \frac{1}{1+e^{-X_i\beta}}
$$
