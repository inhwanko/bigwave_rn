...
$$
\begin{aligned}
P(\theta|\text{y}) = \mathcal{L}(\theta|\text{y})&= \frac{P(\theta)}{P(\text{y})}P(\text{y}|\theta)\\
&= k(\text{y}) \times P(\text{y}|\theta)\\
&\propto P(\text{y}|\theta)
\end{aligned}
$$
...
$$
\beta_0 + \beta_1X_1 + \beta_2X_2 + \dots + \beta_kX_k = X\beta
$$

For instance, in logit regression, our link function is:

$$
log(\frac{\pi_i}{1-\pi_i}) = X_i\beta
$$

This may seem familiar for those who have already used logit regression in one of their previous studies, but any thoughts on what this **actually** means?
...
&=\prod_{i=1}^n \pi^{y_i}(1-\pi_i)^{n-y_i} 
\end{aligned}
$$
...
$$
\begin{aligned}
log [P(y_1, y_2, \dots, y_i|\pi_1, \pi_2, \dots, \pi_i)]
&=log [ \prod_{i=1}^n \pi^{y_i}(1-\pi_i)^{n-y_i} ]\\
&=\sum_i^n[y_i log(\pi_i)] - \sum_i^n[y_i log(1-\pi_i)] + n\sum_i^n log(1-\pi_i)\\
&=\sum_i^n[y_i log(\frac{\pi_i}{1-\pi_i})] + n\sum_i^n log(1-\pi_i)
\end{aligned}
$$
...
The logged odds ratio is the **link function** of the logit regression. Therefore:\newline
...
# From Link Function to MLE: A Case of Logit
\scriptsize

Now, come back to the likelihood:

$$
\begin{aligned}
log\mathcal{L}(\pi_i|y_i) & \propto log P(y_i|\pi_i)\\
&=\sum_i^n[y_i log(\frac{\pi_i}{1-\pi_i})] + \sum_i^n log(1-\pi_i)
\end{aligned}
$$
...
Therefore, our final problem is to find $\pi_i$ that maximizes $logP(y_i|\pi_i)$ which is proportional to the log likelihood, and then put that $\pi_i$ into either one of two equations above and conduct partial derivative to obtain each beta coefficient estimate ($\beta_1, \beta_2, \dots, \beta_k$).
...
And we take the same step as we did in the case of logit. Let's not bother repeating them for now...
...
$$
\frac{\text{odds}(x+1)}{\text{odds}(x)}
=\frac{\text{odds}(x)\times e^{\beta_1}}{\text{odds}(x)}
=e^{\beta_1}
$$
...
$$
(\hat{\beta}-1.96 \times \hat{\sigma_\hat{\beta}},  \hat{\beta}+1.96 \times \hat{\sigma_\hat{\beta}})
$$

2. Exponentiate both ends of the 95% confidence interval of $\hat{\beta}$.

$$(e^{\hat{\beta}-1.96 \times \hat{\sigma_\hat{\beta}}},  e^{\hat{\beta}+1.96 \times \hat{\sigma_\hat{\beta}}})$$
